...
mth{2}=(nu+1)/nu+log(4*nu/pi^2/rho^2)...
       -4*(nu+1)/pi^2/rho^2./avark-log(avark);
% Eq. (A27) in doi: 10.1093/gji/ggt056
mth{3}=-2*nu/rho+8*nu/rho*(nu+1)/pi^2/rho^2./avark;
...
if nargout>1
  ...
  % Checked dmrho/drho
  mththp{3}=2*nu*(1-3*(nu+1)*vpiro./avark+2*nu*(nu+1)*vpiro^2./avark.^2)/rho^2;
  ...
  mththp{6}=2/rho*(-1+[2*nu+1]*vpiro./avark-nu*[nu+1]*vpiro^2./avark.^2);
else 
  mththp=NaN;
end
