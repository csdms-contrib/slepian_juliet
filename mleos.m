...
if ~isstr(Hx)
  ...
  defval('aguess',[1e24 0.8 [var(Hx) 3.0 sqrt(prod(dydx.*NyNx))/5]);
...
elseif strcmp(Hx,'demo1')
  ...
  for index=1:N
    ...
    if index==1
      oswzerob(fid(1),th0,p,ops,bnds,fmts)
    end
	...
	if isreal([logli gr(:)']) ...
	  ...
	  fprintf(fid(2),fmts{1},thhat.*scl);
	  fprintf(fid(3),fmts{1},thini.*scl);
	  ...
	  oswdiag(fid(4),fmts,lpars,thhat,thini,scl,ts,e,o,....
		  var(Hx),momx,covh)
	end
    end
  end
  ...
  if good>=1
    ...
    if labindex==1
      oswzeroe(fid(1),sclth0,avH,good,F,covF,fmti)
    end
  end
